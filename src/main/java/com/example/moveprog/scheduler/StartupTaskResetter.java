...
public class StartupTaskResetter implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        int countDetail = detailRepo.resetStatus(DetailStatus.TRANSCODING, DetailStatus.NEW);
        if (countDetail > 0) {
        ...
        int countLoad = splitRepo.resetStatus(CsvSplitStatus.LOADING, CsvSplitStatus.WAIT_LOAD);
        if (countLoad > 0) {
            log.warn("检测到 {} 个分片在装载中途崩溃，已重置为 WAIT_LOAD。", countLoad);
        }

        // 3. 恢复 Split: VERIFYING -> WAIT_VERIFY
        int countVerify = splitRepo.resetStatus(CsvSplitStatus.VERIFYING, CsvSplitStatus.WAIT_VERIFY);
        if (countVerify > 0) {

...
​			`app.migration-thread-pool.transcode`

###### 			装载

​			`app.migration-thread-pool.load`

###### 			验证	

​			`app.migration-thread-pool.verify`
...
​	`preSqlList`装载前预先执行`sql`

​	`urlOptions` 装载`jdbc url`配置
...
​		`USE_SOURCE_FILE`: 源文件和`tdsql`数据进行比对, `USE_UTF8_SPLIT`用拆分文件和`tdsql`数据进行验证
...
目标数据库URL： jdbc:mysql://192.168.37.101:3306/test2
```



说明：按每个分片建迁移作业，每个分片的作业监控目标不一样(也行在不同的迁移机器上)

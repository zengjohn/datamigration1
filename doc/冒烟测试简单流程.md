...
目标schema: test2
目标库host: 192.168.37.101
目标库端口: 3316
```

其中：目标schema为默认目标库，如果表schema没有指定目标库就用默认库。



说明：

 1. 一个迁移作业只需要在界面创建一个迁移作业，并不需要为每台机器创建一个迁移作业

 2. 同一个迁移作业，监控目录和输出目录， 各个迁移机器上都要存在相同的目录名
...
|                     | `clusterNodes`     |                         |                | 集群所有节点的 IP 列表 (用于监控聚合和广播指令), 格式: 192.168.37.101:8080,192.168.37.102:8080 |              |                                     |
|                     | `job`              |                         |                |                                                              |              |                                     |
...
|                     |                    |                         | max-size       |                                                              |              |                                     |
|                     |                    |                         | queue-capacity |                                                              |              |                                     |
|                     |                    | load-concurrency        |                | 装载**虚拟**线程数, 如果为-1根据`目标库连接池`配置自动计算， 连接池maxPoolSize*0.6， 该参数可以从界面动态调整 |              | -1                                  |
|                     |                    | verify-concurrency      |                | 验证**虚拟**线程数, 如果为-1根据`目标库连接池配置`自动计算， 连接池maxPoolSize*0.3, 该参数可以从界面动态调整 |              | -1                                  |
|                     |                    |                         |                |                                                              |              |                                     |
